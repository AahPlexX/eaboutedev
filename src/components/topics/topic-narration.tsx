...
interface PendingRequest { index: number; playWhenReady: boolean; seekFraction: number; }
interface PassageRequestOptions { playWhenReady: boolean; seekFraction: number; }
...
export function TopicNarration({ topic }: { topic: TopicDocument }) {
  ...
  const setPassageIndex = useCallback((index: number) => { currentIndexRef.current = index; setCurrentIndex(index); setCurrentFraction(0); }, []);
  const failPlayback = useCallback((message: string) => { desiredPlayingRef.current = false; setStatus("error"); setStatusMessage(message); }, []);
  ...
  const loadCachedPassage = useCallback((index: number, url: string, play: boolean, seekFraction = 0) => {
    ...
    const finishLoad = () => {
      ...
      if (!play || !desiredPlayingRef.current) { setStatus("paused"); setStatusMessage(`Paused at passage ${index + 1} of ${passages.length}.`); return; }
  ...
  const requestPassage = useCallback((index: number, options: PassageRequestOptions) => {
    ...
    if (cached) { if (index === currentIndexRef.current) loadCachedPassage(index, cached, options.playWhenReady, options.seekFraction); return; }
    ...
    if (existingRequestId) {
      ...
      if (existing && options.playWhenReady) pendingByRequestRef.current.set(existingRequestId, { ...existing, playWhenReady: true, seekFraction: options.seekFraction });
      return;
    }
    ...
    if (options.playWhenReady) { setStatus("loading"); setStatusMessage(audioUrlsRef.current.size === 0 ? "Preparing the neural voice…" : `Preparing passage ${index + 1}…`); }
  ...
  const handleWorkerMessage = useCallback((event: MessageEvent<WorkerResponse>) => {
    ...
    if (message.type === "progress") { if (message.progress !== undefined) setLoadProgress(message.progress); setStatusMessage(message.progress === undefined ? "Downloading the neural voice…" : `Downloading the neural voice… ${message.progress}%`); return; }
    if (message.type === "ready") { setLoadProgress(null); return; }
    ...
    if (message.type === "error") { if (pending.playWhenReady || pending.index === currentIndexRef.current) failPlayback(`High-quality narration is unavailable right now. ${message.message}`); return; }
    ...
    if (pending.index === currentIndexRef.current) { loadCachedPassage(pending.index, url, pending.playWhenReady && desiredPlayingRef.current, pending.seekFraction); if (pending.playWhenReady && desiredPlayingRef.current) prefetch(pending.index); }
  ...
  const playCurrentPassage = useCallback(() => { desiredPlayingRef.current = true; requestPassage(currentIndexRef.current, { playWhenReady: true, seekFraction: currentFraction }); }, [currentFraction, requestPassage]);
  const togglePlayback = useCallback(() => {
    const audio = audioRef.current;
    if (status === "playing") { desiredPlayingRef.current = false; audio?.pause(); setStatus("paused"); setStatusMessage(`Paused at passage ${currentIndexRef.current + 1} of ${passages.length}.`); return; }
    if (status === "loading" && desiredPlayingRef.current) { desiredPlayingRef.current = false; setStatus("paused"); setStatusMessage(`Paused before passage ${currentIndexRef.current + 1}.`); return; }
    if (status === "complete") { setPassageIndex(0); desiredPlayingRef.current = true; requestPassage(0, { playWhenReady: true, seekFraction: 0 }); return; }
    playCurrentPassage();
  }, [passages.length, playCurrentPassage, requestPassage, setPassageIndex, status]);
  const restart = useCallback(() => { desiredPlayingRef.current = true; setPassageIndex(0); requestPassage(0, { playWhenReady: true, seekFraction: 0 }); }, [requestPassage, setPassageIndex]);
  const seekTo = useCallback((index: number, seekFraction: number) => {
    ...
    if (status === "idle" && !shouldPlay) { setStatusMessage(`Ready at passage ${index + 1} of ${passages.length}.`); return; }
  ...
  const handleRangeChange = useCallback((value: string) => { if (passages.length === 0) return; const numeric = Math.max(0, Math.min(passages.length, Number(value))); const index = Math.min(Math.floor(numeric), passages.length - 1); seekTo(index, Math.max(0, Math.min(0.999, numeric - index))); }, [passages.length, seekTo]);
  const handleEnded = useCallback(() => { if (!desiredPlayingRef.current) return; const nextIndex = currentIndexRef.current + 1; if (nextIndex >= passages.length) { desiredPlayingRef.current = false; setCurrentFraction(1); setStatus("complete"); setStatusMessage("Narration complete."); return; } setPassageIndex(nextIndex); requestPassage(nextIndex, { playWhenReady: true, seekFraction: 0 }); }, [passages.length, requestPassage, setPassageIndex]);
  const handleTimeUpdate = useCallback(() => { const audio = audioRef.current; if (!audio || !Number.isFinite(audio.duration) || audio.duration <= 0) return; setCurrentFraction(Math.max(0, Math.min(1, audio.currentTime / audio.duration))); }, []);
